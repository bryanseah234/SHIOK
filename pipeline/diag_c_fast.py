...
def run_c_fast():
    ...
    for area in pilot_names:
        pa_geom = boundaries_gdf[boundaries_gdf["PLN_AREA_N"].str.upper() == area]
        ...
        if edges is None or edges.empty:
            print(f"{area}: No OSM edges found")
            continue
        ...
        distances = pa_linkways.geometry.distance(osm_union)
        ...
        print(f"\n=== C.2 Polygon-direct match test (3m tolerance) [{area}] ===")
        osm_buffer_3m = osm_union.buffer(3.0)
        pa_linkways["c2_intersection"] = pa_linkways.geometry.intersection(osm_buffer_3m).area
        pa_linkways["c2_matched"] = (
            pa_linkways["c2_intersection"] / pa_linkways.geometry.area
        ) >= 0.5
        pa_linkways["perimeter_div_2"] = pa_linkways.geometry.length / 2.0

        matched_len = pa_linkways[pa_linkways["c2_matched"]]["perimeter_div_2"].sum()
        total_len = pa_linkways["perimeter_div_2"].sum()
        pct = (matched_len / total_len * 100) if total_len > 0 else 0
        print(f"  Match% = {pct:.2f}% ({matched_len:.2f} m / {total_len:.2f} m)")

        print(f"\n=== C.3 Length-estimator cross-check [{area}] ===")
        pa_linkways["mbr_axis"] = pa_linkways.geometry.apply(lambda p: approx_centerline(p).length)
        total_perim2 = pa_linkways["perimeter_div_2"].sum()
        total_mbr = pa_linkways["mbr_axis"].sum()
        print(f"  Perimeter/2 = {total_perim2:.2f} m | MBR-axis = {total_mbr:.2f} m")
