...
- 2026-08-02 - **Train station code source promotion:** `train_station_codes` now uses the official LTA DataMall static `Train Station Codes and Chinese Names.zip` workbook instead of the older data.gov.sg CSV. The DataMall workbook includes newer TEL stations such as Mayflower (`TE6`), while the earlier CSV left Mayflower unlabelled. This is static station metadata only; live arrivals, train direction, and first/last train remain out of the current frontend contract.

- 2026-08-02 - **URA dwelling-unit source added to postal universe:** `ura_no_dwelling_units` now uses official data.gov.sg dataset `d_be71daeab5930f96b90ad2857454d876` as an `official_current` postal-universe source. Probe counts: 83,541 valid URA postals, 75,947 URA-only in `official_current`, raising the official-current probe to 105,462 ready postals. In the current candidate production mode, URA raises total postals from 124,032 to 124,443; bounded OneMap geocode fills 99 of 575 source-derived gaps, leaving 123,967 ready-to-score and 476 unresolved. This improves official-source provenance but does not solve the canonical ~140k postal-universe gap.


- 2026-08-03 - **Bus-route honesty release set extended to 8 postals; production release held on Vercel quota; OneMap walk-validation p95 still failing:** Replayed the top 50 bus-stop overpermissive OneMap outliers under current code; current guards downgraded 18/50 best routes to direct-bus fallback. Selected only same-stop honesty corrections and added `596301`, superseding the 7-postal candidate with an 8-postal release set: 099281, 417471, 596301, 619402, 758588, 760468, 789968, 804651. Bundle `generated_20260803_safe_bus_route_honesty50_targeted` (124,032 records: 112,905 SCORED, 1,422 SCORED_PARTIAL, 9,386 NO_TRANSIT_IN_RANGE, 319 NOT_YET_SCORED) passes all local gates: `compare-targeted` `blocking_count=0` with all 8 postals marked `safe_promotable`, static validation ok (3,171 files, 6,011 transit features, 0 errors, 0 warnings), score-provenance manifest ok (5 scoring fingerprints, 0 missing), readiness ok with 0 warnings, and full local `launch-check.ps1` ok (Python tests, web tests, fresh-bundle web build, readiness, 5 browser smokes, plan-only release). Production release is held on Vercel Hobby `api-deployments-free-per-day` hit at 2026-08-03T05:52 SGT (evidence: `qa/release_attempt_safe_recovered_validation_20260803.json`); retry after the 24-hour quota window resets by re-running `scripts\targeted_bundle_refresh.py` + `scripts\release-data-bundle.bat -DataBundle generated_20260803_safe_bus_route_honesty50_targeted -ConfirmProduction`. OneMap 2000-postal cached walk-validation against the new bundle still fails release thresholds: median absolute delta 11.582% (max 10.0%), p95 103.196% (max 25.0%). Bus-stop routing is the primary driver (1,611 samples, median 12.893%, p95 112.444%, 524 over 25%); MRT is within median (6.694%) but fails p95 (58.926%). This is a real data-quality gap, not a stale-bundle artefact - the 50-outlier corrections improved median by 0.14pp and p95 by 2.99pp versus the prior `safe_mayflower_560234_targeted` cached run. Wider current-code replays of the top bus-stop outliers where the project route is materially shorter than OneMap wrote remain the next quality lever; readiness treats the OneMap gate as advisory rather than a hard blocker, so tomorrow's release/hold decision on this bundle is a data-quality judgment call, not a mechanical gate.
