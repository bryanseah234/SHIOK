...
- Complete the 2,000-postal OneMap validation gate from PRD §2/§12: stratified sample, cached OneMap walk-route responses, median ≤ 10%, p95 ≤ 25%, and publish blocked if thresholds fail.
  Current implementation note (2026-08-02): `run.py onemap-validation plan` generates the deterministic 2,000-postal sample, `collect` writes the guarded/resumable OneMap route cache only with `--confirm-onemap-collection`, and `evaluate` checks cached responses; the passing external gate run is still pending.
...
- Run and fix the Lighthouse/accessibility pass: homepage Lighthouse a11y is 100 in `qa/lighthouse_a11y_20260802_summary.md`; routed-search desktop/mobile smoke screenshots for `560234` are recorded in `qa/browser_qa_560234_20260802.md`. A broader manual keyboard-only and multi-postal mobile pass is still pending.
