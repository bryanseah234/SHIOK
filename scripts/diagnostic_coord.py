...
def run_diagnostic():
    ...
    print(f"\nDropped ways touching this component: {len(touching_dropped)}")
    for i, row in enumerate(touching_dropped[:5]):
        print(
            f"  [{i}] Highway: {row.get('highway', 'NA')}, Access: {row.get('access', 'NA')}, Foot: {row.get('foot', 'NA')}"
        )

    main_comps = comps_filtered[:3]
    main_geom = gpd.GeoSeries([Point(n) for comp in main_comps for n in comp]).union_all()
    comp_geom_pts = gpd.GeoSeries([Point(n) for n in target_comp]).union_all()

    from shapely.ops import nearest_points

    p_comp, p_main = nearest_points(comp_geom_pts, main_geom)
    gap = p_comp.distance(p_main)
    print(f"\nGap distance to top-3 components: {gap:.2f}m")
    print(
